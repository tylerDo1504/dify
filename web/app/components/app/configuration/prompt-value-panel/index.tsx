...
const PromptValuePanel: FC<IPromptValuePanelProps> = ({
  ...
  return (
    <div
      className="pb-3 border border-gray-200 bg-white rounded-xl"
      style={{
        boxShadow:
                ...
                {promptVariables.map(
                  ({ key, name, type, options, max_length, required }) => (
                    <div key={key} className="xl:flex justify-between">
                      <div className="mr-1 py-2 shrink-0 w-[120px] text-sm text-gray-900">
                        {name || key}
                      </div>
...
function replaceStringWithValuesWithFormat(
  ...
  return str.replace(/\{\{([^}]+)\}\}/g, (match, key) => {
    ...
    return `<div class='inline-block px-1 rounded-md text-gray-500' style='background: rgba(16, 24, 40, 0.05)'>${
